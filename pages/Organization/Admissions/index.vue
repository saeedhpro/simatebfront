    ...
    <vue-modaltor :visible="open" @hide="open=false">
      ...
      <template #body>
        ...
        <div class="m-2 flex justify-start" style="align-items: center">
          <div class="cursor-pointer px-3 py-2 ml-2 flex bg-white border-gray-901 border-solid rounded-lg border"
               :class="{'selectedBlue' : status1 === '1'}">
            <div style="align-items: center">
              <input v-model="caseType" type="radio" value="جراحی" class="w-5 bg-gray-907 rounded-full">
              <span>جراحی</span>
            </div>
          </div>
          <div class="cursor-pointer px-3 py-2 ml-2 flex bg-white border-gray-901 border-solid rounded-lg border"
               :class="{'selectedBlue' : status1 === '1'}" style="align-items: center">
            <input v-model="caseType" type="radio" value="معاینه" class="w-5 bg-gray-907 rounded-full">
            <span>معاینه</span>
          </div>
          <div class="cursor-pointer px-3 py-2 ml-2 flex bg-white border-gray-901 border-solid rounded-lg border"
               :class="{'selectedBlue' : status1 === '1'}" style="align-items: center">
            <input v-model="caseType" type="radio" value="seperator" class="w-5 bg-gray-907">
            <span>seperator</span>
          </div>
          <div class="cursor-pointer px-3 py-2 ml-2 flex bg-white border-gray-901 border-solid rounded-lg border"
               :class="{'selectedBlue' : status1 === '1'}" style="align-items: center">
            <input v-model="caseType" type="radio" value="بند" class="w-6  bg-gray-907">
            <span>بند</span>
          </div>
          <div class="cursor-pointer px-3 py-2 ml-2 flex bg-white border-gray-901 border-solid rounded-lg border"
               :class="{'selectedBlue' : status1 === '1'}" style="align-items: center">
            <input v-model="caseType" type="radio" value="دی باند" class="w-5 bg-gray-907 rounded-full">
            <span>دی باند</span>
          </div>
        </div>
    ...
    <div>
      <div class="flex flex-wrap justify-between">
        ...
        <button @click="status1" class="px-3 py-2 flex  bg-white rounded-lg shadow-md text-sm click"
          ...
          {{ canApp() ? 'پذیرش های امروز' : 'ارسال نتایج'}}
        </button>
      ...
      <div v-if="menu1">
        <AdmissionsToday v-if="canApp()"/>
        <SendDocToday v-else :prof="prof_id" />
      </div>
      <div v-if="menu2">
        <AdmissionsSearch/>
      </div>
...
<script>
import AdmissionsToday from "/components/DoctorProfile/AdmissionsToday";
import AdmissionsSearch from "/components/DoctorProfile/AdmissionsSearch";
import SendDocToday from "../../../components/DoctorProfile/SendDocToday";
...
export default {
  ...
  components: {
    SendDocToday,
    "app-component-1": AdmissionsToday,
    "app-component-2": AdmissionsSearch,
    AdmissionsToday, AdmissionsSearch
  },
  methods: {
    openModal() {
      if (this.canApp()) {
    ...
    saveAppointment() {
      if (!this.start_at || !this.user) return;
