<template>
  <div class="container">
    ...
    <vue-modaltor :visible="openPaziresh" @hide="openPaziresh=false">
      <template #header>
        <div class="pb-3 flex justify-between border-b border-gray-700" style="align-items: center">
          <div class="flex" style="align-items: center">
            <div class="w-5 h-5 p-1 border-solid border-gray-901 border rounded">
      ...
      <template #body>
        <div>
          <div class="my-5 flex flex-wrap justify-between">
            ...
            <div>
              ...
              <input disabled :value="getUser ? getUser.tel : ''"
                     class="w-56 h-10 px-2 bg-white border-solid border-gray-901 border rounded-lg"/>
            </div>
          </div>
          <div class="flex mb-5 flex-wrap justify-between">
            <div>
              ...
              <input disabled class="w-56 h-10 px-2 bg-white border-solid border-gray-901 border rounded-lg"
                     :value="getUser ? getUser.nid : ''"/>
            </div>
            <div>
              <div class="mb-2 Bold text-sm">شماره پرونده</div>
              <input disabled class="w-56 h-10 px-2 bg-white border-solid border-gray-901 border rounded-lg"
                     :value="getUser ? getUser.file_id : ''"/>
            </div>
            <div>
        ...
        <div class="mt-8 flex flex-wrap justify-between">
          ...
          <div class="flex" style="align-items: center">
            <button class="w-28 h-10 py-2 text-center bg-gray-907 rounded-lg" @click="toggleOpenPaziresh">انصراف</button>
    ...
    <vue-modaltor v-if="getUser" :visible="save" @hide="save=false">
      <template #header>
        <!--    add your custom header     -->
        <div class="pb-3 flex justify-between" style="align-items: center">
          <div class="flex" style="align-items: center">
            <div class="w-5 h-5 p-1 border-solid border-gray-901 border rounded">
              <img src="/img/xmark.svg" @click="save = false">
            </div>
            <!--          <p class="mr-2">فرم پذیرش</p>-->
          </div>
        </div>
      ...
      <template #body>
        ...
        <div>
          ...
          <div class="my-3 text-blue-907 text-center text-sm">آیا از رزرو وقت برای <span
            class="Bold">{{ `${getUser.fname} ${getUser.lname}` }}</span> در روز <span
        ...
        <div class="mt-8 flex flex-wrap justify-between">
          <div class="w-1/2 h-10 px-2">
            <button class="w-full py-2 text-center bg-gray-907 rounded-lg" @click="save = false">خیر</button>
          </div>
          <div class="w-1/2 h-10 px-2">
          ...
          <div v-for="(i,n) in (daysInMonth)" class="day-col">
            ...
            <div class="appt-day-col">
              <div class="appt-day-col-header"
                   :class="{'is-friday' : isFriday(i), 'is-holiday': isHoliDay(i)}">
              ...
              <div class="appt-day-col-content">
                <div v-for="(k,l) in workTimeItemCount" class="appt-day-col-content-item"
                  ...
                  <div class="appt-day-col-content-time">
                    <div v-if="showTime">
                      <div v-if="apps[n] && apps[n][l]">
                        <AppointmentReserveItem v-if="apps[n] && apps[n][l]" @selected="itemClicked" :item="apps[n][l]"/>
                      </div>
                      <div v-else @click="openForTime(getTimeString(l), i)" class="cursor-pointer">{{ getTimeString(l) }}</div>
  ...
  methods: {
    ...
    saveAppointment() {
      this.$store.dispatch('doctor/patients/createPaziresh', {
        ...
        organization_id: parseInt(this.getUser.organization_id),
        is_vip: 0,
    ...
    async getHolidayList() {
      const start_date = this.$moment(`${this.year}/${this.month}/01`, 'jYYYY/jMM/jDD').format("YYYY/MM/DD")
      const end_date = this.$moment(`${this.year}/${this.month}/${this.daysInMonth}`, 'jYYYY/jMM/jDD').format("YYYY/MM/DD")
      const holiDays = await this.$store.dispatch('admin/holiday/getOrganizationHolidayList', {
        id: this.userLogin.organization_id,
        start_date: start_date,
        end_date: end_date,
      })
      for (let i = 0; i < holiDays.length; i++) {
        if (!holiDays[i].organization_id) {
          this.holiDays.push(holiDays[i])
        } else {
          if (holiDays[i].organization_id == this.userLogin.organization_id) {
            this.holiDays.push(holiDays[i])
          }
        }
      }
    ...
    openForTime(time, day) {
      this.onChange(this.$moment(`${this.year}/${this.month}/${day} ${time}`, "jYYYY/jMM/jDD HH:mm:ss").format("YYYY/MM/DD HH:mm:ss"))
      this.toggleOpenPaziresh()
    },
    clean() {

    },
    toggleOpenPaziresh() {
      this.openPaziresh = !this.openPaziresh
    },
    setDent(f, index) {
      if (f == 1) {
    ...
    updateApp() {
      ...
      console.log(rads)
      console.log(this.photography_id)
      console.log(this.radiology_id)
      const data = {
      ...
      for (let i = 0; i < this.daysInMonth; i++) {
        ...
        for (let j = 0; j < appointments.length; j++) {
          ...
          if (time.isBetween(startTime, endTime)) {
            ...
            this.times.push(this.getTimeString(i))
          }
        }
        apps.push(founded)
      }
    ...
    isHoliDay(time) {
      for (let i = 0; i < this.holiDays.length; i++) {
        // const date = parseInt(this.$moment(this.holiDays[i]).format("jDD"))
        const date = this.$moment(this.$moment(this.holiDays[i].hdate).format("YYYY/MM/DD")).format("jYYYY/jMM/jDD")
        const t = `${this.year}/${this.month}/${time}`
        if (date == t) {
          return true
        }
      }
